...
function perkalianUnik(arr) {
  //membuat variabel untuk menampung hasil
	var hasil = [];
	//looping untuk mengecek angka
	for(var i= 0; i < arr.length; i++) {
	//membuat variabel baru untuk menampung perkalian
	var tampung = 1;
	//looping lagi untuk mengkali kan angka-angka lain setelah angka loopingan pertama
		for (var j= 0; j< arr.length; j++) {
		//membuat pengkondisian untuk memastikan angka yg dikali adalah bukan arr[i]
			if (i !== j) {
				//mengkalikan angka2 hasil looping
				tampung *= arr[j]
			}
		}
		hasil.push(tampung);

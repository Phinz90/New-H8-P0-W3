...
function mengelompokkanAngka(arr) {
	...
	var ganjil = [];
	var kelipatan3 = [];
	//memulai looping untuk pengecekan arr[i]
	for (var i = 0; i < arr.length; i++) {
		//membuat kondisi untuk mencakup array kelipatan 3
		if (arr[i] % 3 === 0) {
			kelipatan3.push(arr[i]);
			//membuat kondisi lain untuk mencakup array genap
		} else if (arr[i] % 2 === 0) {
			genap.push(arr[i]);
			//membuat kondisi lain untuk mencakup array ganjil
		} else if (arr[i] % 1 === 0) {
			ganjil.push(arr[i]);
		}
